...
from sap import SAPServer, make_object, timestamp, link, Scope, load_xml
import time
import random
import uuid

def fetch_data():    
    xml_objects = load_xml(
        file_path="examples/example.xml",
        source="sample_provider",
        type_name="layout",
        root_element_id="layout"
    )
    ...
    if scope.type == "simple":
        return [
            make_object(
                ...
                properties={"one": "one", "employee": link("employee#emp_001", "Alice Johnson")},
            ),
        ], "Plan"
    elif scope.type == "complex":
        return [
            make_object(
                id="complex_001",
                types=["complex"],
                source="complex_provider",
                properties={"one": "one", "two": "two", "three": "three"},
            ),
        ], "Plan"
...
if __name__ == "__main__":
    lazy_scopes = [
        ...
        Scope(type="complex", fields="*", filtering_fields=[], needs_id_types=False)
    ]
